...
function loadConfig(vendor: string): CrawlConfig {
  ...
  if (!existsSync(path)) {
    const vendorDir = resolve(VENDOR_ROOT, vendor);
    if (existsSync(vendorDir)) {
      // Study clones (e.g. vendor/git/) live under vendor/ as read-only
      // upstream snapshots, not as refreshable mirrors. listVendorConfigs()
      // filters them out, but a direct --vendor flag bypasses that filter.
      throw new Error(
        `vendor/${vendor} is a study clone with no crawl.json — not refreshable. ` +
          `Run \`npm run crawl:vendors\` (no --vendor) to crawl every refreshable vendor.`,
      );
    }
    throw new Error(
      `no such vendor: vendor/${vendor}/ does not exist. ` +
        `Run \`npm run crawl:vendors\` to list refreshable vendors.`,
    );
  }
