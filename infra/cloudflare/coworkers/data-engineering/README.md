# data-engineering — Cloudflare Worker (wrangler side)

Per-coworker Worker for the `data-engineering` managed-coworker. TypeScript-side parallel of the Terraform module at `infra/terraform/coworkers/data-engineering/` (PR #118, ODEP3). Both define the same Cloudflare resources; at deploy time the operator picks one as the source of truth.
...
src/worker.ts    — Worker entrypoint + env-sanitizer + DO stubs
                   (full skill-dispatch surface lands in ODEP3c)
README.md        — this file
```
...
| `SESSION_INDEX` | KV | Hot mirror of `coworker:data-engineering:session:<id>` |
...
```bash
cd infra/cloudflare/coworkers/data-engineering
...
Gated on the chassis-wide deploy decision (Phase 8) + operator secret sync. The bootstrap workflow at `.github/workflows/cloudflare-preview.yml` (chassis-wide) provides the pattern; per-coworker equivalent lands when ODEP3c does.

## Sibling

The Terraform equivalent at `infra/terraform/coworkers/data-engineering/` (PR #118) defines the same resources. The sibling `data-engineering` Terraform module is PR #118. The `data-engineering` wrangler.jsonc equivalent will follow this same pattern when authored.
