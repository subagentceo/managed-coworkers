/**
 * Per-coworker Worker for the product-management managed-coworker.
 *
 ...
 * session DO + AE logging surface lands in a follow-up (OPMP3c).
 *
 * Refs: OPMP3b.
 *
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    return new Response("coworker-product-management: stub. See OPMP3c for the skill-dispatch surface.", {
      status: 200,
...
export class Sandbox {
  ...
  async fetch(): Promise<Response> {
    return new Response("Sandbox DO stub — image: ./Dockerfile (OPMP3b)", { status: 200 });
  }
}
