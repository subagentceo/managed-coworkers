# Cloudflare resources for the product-management managed-coworker.
#
# Parallels (does not yet replace) the wrangler.jsonc path planned at
# infra/cloudflare/coworkers/product-management/wrangler.jsonc (PR C of
# OPMP). When both land, the operator picks one deploy path; the other
# stays as drift-detection / reference.
#
# Refs: OPMP3.
...
# the planned wrangler.jsonc binding set. Stores `coworker:product-management:session:<id>`
