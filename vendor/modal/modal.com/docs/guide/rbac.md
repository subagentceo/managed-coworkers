...
RBAC is available on the <a href="/pricing">Enterprise plan</a>. Contact <a href="mailto:sales@modal.com">sales@modal.com</a> for more information.
</Callout>
...
1. **Enable RBAC**: This requires an Enterprise plan. See our [pricing page](/pricing) for more information.

2. **Create a restricted Environment**: Navigate to your Workspace's Environment Management page in [Settings](/settings) and click **Add Environment**.
...
## Proxy auth tokens for Web Functions

[Web Functions](/docs/guide/webhooks) can be protected with proxy auth tokens, which authenticate inbound HTTP requests before they reach your function.

On workspaces with RBAC enabled, proxy auth tokens are **scoped** — each token is explicitly associated with one or more Environments, and will only be accepted for Web Functions deployed to those Environments. This prevents a token intended for a staging endpoint from being used to call a production one.

### Creating a scoped proxy auth token

1. Navigate to **Settings → Proxy Auth Tokens** and click **New Token**.
...
| Scoped proxy auth token used on a Web Function in an associated Environment    | Yes                                    | Token must be explicitly associated with the target Environment                             |
| Scoped proxy auth token used on a Web Function in a non-associated Environment | No                                     | Token is not valid for Environments it has not been associated with                         |
