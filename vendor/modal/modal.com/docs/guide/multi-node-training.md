...
This feature is in beta. We'd love to hear your feedback — please reach out via [Slack](/slack) or email us at <support@modal.com>.
...
Starting May 31st, 2026, clustered functions must use `8` GPU devices per node (e.g. `H100:4` is invalid). Clustered functions require GPUs, and CPU-only functions are not supported. If you have a special case that does not fall under above and would like to use clustered functions, please contact <support@modal.com>.
