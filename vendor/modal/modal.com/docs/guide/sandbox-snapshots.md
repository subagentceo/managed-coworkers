...
| ------------------- | ------------------------------------- |
| Filesystem Snapshot | Indefinite (until explicitly deleted) |
| Directory Snapshot  | 30 days after last use                |
| Memory Snapshot     | 7 days after creation                 |

Filesystem Snapshots as created by `Sandbox.snapshot_filesystem()` are [Images](/docs/reference/modal.Image) and persist indefinitely until you explicitly delete them. Directory Snapshots (created by `Sandbox.snapshot_directory()`) are automatically garbage collected 30 days after they were last created or used. Memory Snapshots expire 7 days after creation and cannot currently be extended.

To manage storage for long-lived Filesystem Snapshots, you can delete them programmatically when no longer needed. See [Deleting Filesystem Snapshots](#deleting-filesystem-snapshots) for details.
...
Filesystem Snapshots as created by `Sandbox.snapshot_filesystem()` persist indefinitely until explicitly deleted. See [Deleting Filesystem Snapshots](#deleting-filesystem-snapshots) to learn how to manage snapshot storage.

### Deleting Filesystem Snapshots

Since Filesystem Snapshots are [Images](/docs/reference/modal.Image), you can delete them using the image deletion API. This is useful for managing storage or complying with data retention policies.
...
# Get the image ID from a snapshot
image = sb.snapshot_filesystem()
...
```go notest
// Get the image ID from a snapshot
image, _ := sb.SnapshotFilesystem(ctx, 55*time.Second)
...
To delete filesystem snapshots, you need to track the image IDs yourself (e.g., in a database or [Modal Dict](/docs/guide/dicts)), since there is currently no API to list all filesystem snapshots you have created.
...
```go notest
_ := sb2.UnmountImage(ctx, "/project")
```
...
Directory Snapshots have a 30-day retention period after they were last created or used (see [Snapshot Retention](#snapshot-retention)). If you try to use an expired snapshot, Modal will raise a `NotFoundError` when attempting to start a sandbox or mount the Image as a directory, letting you handle the case gracefully. Note that `Image.from_id()` is itself lazy and will not raise an error on construction even if the provided image id has been deleted.

## Memory Snapshots (Alpha)

<Callout variant="alpha">

This feature is currently in Alpha and has a number of known [limitations](#limitations). See [feature maturity](/docs/guide/feature-maturity) for more details.
